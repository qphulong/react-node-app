...
const AdminPage = () => {
  ...
  const CheckIsAdmin = async () => {
    try {
      const response = await axios.get(
        window.backendURL + `/user/${currentUser.userId}/admin`
      );
  ...
  const GetAllUsers = async () => {
    try {
      const response = await axios.get(window.backendURL + `/user/admin`);
    ...
    return (
      <div className="not-admin-page">
        <div className="notification">
          You cannot be here. Only Admins have permission to access this page.{" "}
          <br></br>
