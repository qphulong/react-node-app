...
const Profile = () => {
  ...
  const { currentUser, profileImage, setProfileImage } =
  ...
  //===================================================================================================================
  //profile image
  const [profileImageFriend, setProfileImageFriend] = useState(
    ...
    if (currentUser.userId !== userId) {
      const fetchIsFriend = async () => {
        try {
          const response = await axios.get(
            window.backendURL +
              `/user/friends/check/${currentUser.userId}/${userId}`
          );
    ...
    return (
      <div className="not-friend-page">
        <div className="notification">
          You and this user are not friends. You can send a request to invite
          this user to be your friend later. <br></br>
  ...
  const handleClickDisplaySocialMedia = async () => {
    try {
      const response = await axios.get(
        window.backendURL + `/user/social-media/${userId}`
      );
  ...
  const deleteLink = async (social) => {
    try {
      const response = await axios.put(
        window.backendURL + `/user/social-media/${currentUser.userId}`,
        {
  ...
  return (
    <div className="profile">
      <div className="images">
        <img
          src={isOwnProfile ? profileImage : profileImageFriend}
          alt=""
          className="cover"
        />
        <div className="profile-image-container" onClick={handleImageClick}>
          {image ? (
            <img
              ...
              alt=""
              className="profile-picture-after"
            />
          ) : (
            <img
              src={isOwnProfile ? profileImage : profileImageFriend}
              className="profile-picture-before"
            />
          )}
          {isOwnProfile ? (
            <FileUploadIcon
        ...
        <div className="userInfo">
          <div className="center">
            ...
            <div className="social-link">
              <button
                onClick={handleClickDisplaySocialMedia}
                    ...
                    <li key={index} className="list-item">
                      ...
                      {isOwnProfile && (
                        <div className="remove-container">
                          <HighlightOffIcon
                            style={{ fontSize: 30 }}
                            onClick={() => handleRemoveLink(index, social)}
                            className="logo"
                          />
                          <div className="tool-tip">Remove link</div>
                        </div>
                      )}
                    </li>
                  ))}
      ...
      <div className="container-post">
        <PostsProfile
          userId={isOwnProfile ? currentUser.userId : userId}
          imageProfile={isOwnProfile ? profileImage : profileImageFriend}
        />
      </div>
    </div>
  );
